...
def remove_padding_and_shift(packet):
    if TCP in packet or UDP in packet:
        ...
        pad_length = payload[-2]
        shift_amount = payload[-1]
        actual_payload = payload[:-2]

        # Reverse the shift
        original_payload = actual_payload[shift_amount:] + actual_payload[:shift_amount]
        original_payload = original_payload[:-pad_length]  # Remove padding
...
def deobfuscate_traffic(input_pcap, output_pcap):
    ...
    for packet in packets:
        try:
            deobfuscated_packet, pad_length = remove_padding_and_shift(packet)
...
if __name__ == "__main__":
    input_pcap = 'performance/padshift.pcap'
    ...
    # Step: Perform deobfuscation on the padded and shifted packets
    print("Performing deobfuscation on padded and shifted packets...")
