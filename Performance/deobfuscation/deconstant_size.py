...
def remove_padding(packet):
    if TCP in packet or UDP in packet:
        ...
        if len(payload) < 3:  # Check if payload is large enough to contain metadata
            return packet, 0  # Return the original packet if it's too small

        padding_length = int.from_bytes(payload[-2:], 'big')
        if padding_length > len(payload) - 2:
            return packet, 0  # Return the original packet if padding length is invalid

        original_payload = payload[:-2 - padding_length]
...
def deobfuscate_traffic(input_pcap, output_pcap):
    ...
    for packet in packets:
        try:
            deobfuscated_packet, padding_length = remove_padding(packet)
            deobfuscated_packet.time = time.time()
            deobfuscated_packets.append(deobfuscated_packet)
            total_padding_bytes_removed += padding_length
        except Exception as e:
            print(f"Error deobfuscating packet {packet.number}: {e}")
...
if __name__ == "__main__":
    input_pcap = 'performance/constant.pcap'
    output_pcap = 'performance/deobfuscated.pcap'
    ...
    total_padding_bytes_removed, latency = deobfuscate_traffic(input_pcap, output_pcap)
    end_time = time.time()
    deobfuscation_time = end_time - start_time
    print(f"Deobfuscated packets saved to: {output_pcap}")
